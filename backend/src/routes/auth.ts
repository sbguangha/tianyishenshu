...
// 登录验证
router.post('/verify-login', validateInput, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 模拟验证码验证（开发环境）
    const isCodeValid = process.env.NODE_ENV === 'development' || code === '123456'
    if (!isCodeValid) {
    ...
    // 生成JWT令牌
    const userPayload = {
      id: `user_${phone}_${Date.now()}`, // 使用手机号确保唯一性
      phone: phone
    };
    ...
    const user = {
      ...
      needsUserInfo: true // 需要跳转到用户信息表单页
    }

    console.log(`用户 ${phone} 使用兑换码 ${exchangeCode.toUpperCase()} 登录成功`)
